...
# save-my-tokens Agent Instructions

**Core rule: Never read a full file when the task probably needs a symbol.**

This MCP server provides compact, symbol-first code context. Use it to save tokens and get faster, more precise answers.

## Preferred Tool Order

Always attempt tools in this order — stop as soon as you have sufficient context:

1. `ask_codebase` — start with a broad question or task description
2. `resolve_symbol` — locate a specific symbol by name without bodies
3. `get_symbol_context` — inspect a symbol with optional dependency depth
4. `get_relevant_context` — find task-relevant symbols and files
5. `get_usage_context` — find callers and usages of a symbol
6. `get_exact_snippet` — retrieve exact bounded source lines for edit-critical ranges
7. `read_full_file_escape_hatch` — **only when necessary**, with justification

## Tool Reference

### `ask_codebase`
Answer a broad question using indexed symbols and summaries. Use as the entry point when you have a question rather than a specific symbol name.

### `resolve_symbol`
Locate a symbol by qualified or unqualified name. Returns location, kind, and signature without source bodies. Use before `get_symbol_context` to confirm the exact match.

### `get_symbol_context`
Return symbol-first context at depth 0 (location + signature) or depth 1 (add direct dependencies, `this.*` references, imports). Request `include.body: true` only for edit-critical symbols.

### `explain_symbol_dependencies`
Explain dependency relationships without exact source code. Prefer this over reading files when you need call-flow understanding but not edit targets.

### `get_usage_context`
Find callers and usages of a symbol across the indexed repository. Use for impact analysis before refactoring.

### `get_relevant_context`
Retrieve task-relevant symbols and files via lexical scoring. Use `mode: "edit"` to get edit targets with recommended follow-up calls.

### `get_exact_snippet`
Return exact bounded source lines for a known safe range. Use only for confirmed edit-critical code — not for exploration.

### `get_file_summary`
Return file purpose, imports, exports, and symbols without full content. Prefer this over reading a file when you need an overview.

### `read_full_file_escape_hatch`
Guarded fallback. Provide a non-empty `justification`. The tool rejects files above the line limit, secret-like paths, and paths outside configured roots. On rejection, follow the suggested alternatives.

## Token Budget

Every tool accepts `max_tokens`. Set it to your available context budget. Responses include `token_budget.truncated: true` when content was omitted — follow `next_recommended_calls` to retrieve omitted context.

## Guardrails

- Paths outside configured roots are rejected with `PATH_OUTSIDE_ROOT`.
- Secret-like files (`.env`, `*.pem`, `*.key`) are rejected.
- Dependency depth above the configured maximum returns `DEPENDENCY_DEPTH_TOO_HIGH`.
- Large full-file reads are rejected with symbol alternatives provided.
...
<!-- OMX:RUNTIME:START -->
<session_context>
**Session:** omx-1777635121746-i03pwi | 2026-05-01T11:32:01.790Z

**Codebase Map:**
  src/: cli, server
  src/config/: loadConfig, schema
  src/guardrails/: depth, pathSafety, secretRedaction, tokenBudget
  src/indexer/: buildIndex, cache, dependencyGraph, fileTree, importGraph, symbolIndex, types
  src/languages/: languageAdapter, adapter, dependencies, symbols
  src/local-ai/: client, rankDependencies, summarize
  src/mcp/: registerTools, schemas, askCodebase, common, explainSymbolDependencies, getExactSnippet, getFileSummary, getRelevantContext, getSymbolContext, getUsageContext
  src/retrieval/: contextPlanner, recommendations, relevance
  src/utils/: errors, logger, text
  tests/: duplicates, emailSender, large, repo, service, util, buildIndex.test, tools.test, config.test, pathSafety.test
  (root): eslint.config, vitest.config
