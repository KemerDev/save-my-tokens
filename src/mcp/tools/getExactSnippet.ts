...
export function getExactSnippet(context: RuntimeContext, raw: unknown) {
  return catchTool(async () => {
    ...
    if (requestedLines > maxLines)
      return errText(
        ...
        `Exact snippet range is too large (${requestedLines} lines); narrow to ${maxLines} lines or less`,
        { requestedLines, maxLines, path: input.path },
        [
          {
