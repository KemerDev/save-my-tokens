...
export const localAiSchema = z.object({
  ...
  provider: z.enum(['ollama', 'lmstudio', 'openai-compatible']).default('ollama'),
...
export const saveMyTokensConfigSchema = z.object({
  ...
  excludeDirs: z.array(z.string()).default(['node_modules', 'dist', 'build', '.next', 'coverage', '.git', 'vendor']),
  excludeFiles: z.array(z.string()).default(['.env', '.env.*', '*.pem', '*.key', '*.crt']),
