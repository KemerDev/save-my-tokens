...
function reserve(b: TokenBudget, text: string, required: boolean, priority?: DependencyImportance): BudgetReservation { const estimatedTokens = estimateTokens(text); const slack = priority === 'high' ? 0 : priority === 'medium' ? 20 : 80; if (estimatedTokens + slack <= b.remainingTokens) { b.remainingTokens -= estimatedTokens; b.consumedTokens += estimatedTokens; return { accepted: true, estimatedTokens }; } if (required && estimatedTokens <= b.requestedMaxTokens) { b.remainingTokens -= estimatedTokens; b.consumedTokens += estimatedTokens; b.truncated = true; return { accepted: true, estimatedTokens }; } b.truncated = true; return { accepted: false, estimatedTokens, reason: 'token budget exceeded' }; }
