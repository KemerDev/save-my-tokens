...
export function getSymbolContext(context, raw) {
    return catchTool(async () => {
        const input = getSymbolContextInputSchema.parse(raw);
        const depth = validateDependencyDepth(input.dependency_depth, context.config);
        if (!depth.allowed)
        ...
        const canIncludeBody = body && symbolLines <= context.config.maxExactSnippetLines;
        const snippet = canIncludeBody
            ? await snippetForRange(context, resolved.path, resolved.range)
            : null;
        const deps = context.index.dependencies.filter((d) => d.fromSymbolId === resolved.id);
        const imports = context.index.imports.filter((i) => i.path === resolved.path);
        const included = input.dependency_depth > 0
            ? deps
                .slice(0, 12)
                .map((d) => ({
                symbol: d.reference,
            ...
            : [];
        const bodyOmittedReason = body && !canIncludeBody
        ...
        return {
            ...common(input.max_tokens, parts, [
                recommendSnippet(resolved.path, resolved.range.startLine, resolved.range.endLine, bodyOmittedReason ?? "Retrieve exact edit range", context.config.maxExactSnippetLines),
            ], "medium"),
            primary_symbol: {
                ...
                summary: bodyOmittedReason ?? `${resolved.kind} ${resolved.qualifiedName}`,
            },
            ...(() => {
                const selfRefDeps = deps.filter((d) => d.relationship === "calls" && !d.reference.startsWith("this.") && d.resolution !== "external");
...
//# sourceMappingURL=getSymbolContext.js.map