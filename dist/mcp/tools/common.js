...
export function okText(data) { return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }], structuredContent: data }; }
export function errText(code, message, details = {}, suggested = []) { const data = toolError(code, message, details, suggested); return { content: [{ type: 'text', text: JSON.stringify(data, null, 2) }], structuredContent: data, isError: true }; }
...
export function resolveOne(context, symbol, fileHint) { const matches = findSymbols(context.index, { symbol, fileHint, maxResults: 10 }); if (matches.length === 0)
