...
export function registerTools(server, context) {
    ...
    for (const [name, schema, handler] of specs)
        server.registerTool(name, { title: name, description: descriptions[name], inputSchema: schema.shape }, async (args) => handler(await ctx, args));
}
export function registeredToolNames() { return [...TOOL_NAMES]; }
