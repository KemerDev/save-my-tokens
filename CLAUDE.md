...
### native file reads — BLOCKED

`Read` on source files for analysis floods context. Use `get_file_summary`, `get_symbol_context`, or `get_exact_snippet` instead.  
Exception: `Read` is correct when you are about to `Edit` a file (Edit needs the content in context).

### native search — BLOCKED

`Grep` / `Glob` / `Bash find` for source exploration floods context. Use `ask_codebase`, `get_relevant_context`, or `resolve_symbol` instead.

### automatic validation — BLOCKED

Do NOT run lint, build, typecheck, test, or format checks unless the user explicitly requests them. Summarize the edit and offer manual commands instead.

## REDIRECTED — use MCP tools

### Bash / Grep / Glob on source
...
### Full-file read
→ `read_full_file_escape_hatch` with non-empty justification. State why symbols and snippets were insufficient.

## Tool selection

Stop at the first tool that answers the question. Later tools cost more tokens.
...
Batch independent lookups in the same response turn:
...
Terse. Write artifacts to files — never inline. Return: file path + one-line description.  
...
If the response contains `"truncated": true`, follow `next_recommended_calls` — do **not** fall back to native reads.
